...
where $\pi$ denotes conditional probability of observing singletons.
...
\nlset{2} Store values from the previous step: 
    ...
    $\displaystyle\boeta\leftarrow\bX_{\text{vlm}}\bbeta+\bo, \bW_{(k)}\leftarrow\mathbb{E}\left[-\frac{\partial^{2}\ell}{\partial\boeta_{(k)}^\top\partial\boeta_{(k)}}\right], Z_k\leftarrow\boeta_{(k)}+\frac{\partial\ell}{\partial\boeta_{(k)}}\bW_{(k)}^{-1}-\bo_{(k)}$,\;
...
We have implemented three types of bootstrap algorithms: parametric, semi-parametric and nonparametric. The nonparametric version is the usual bootstrap algorithm, as noted in \cite{norrpoll} and \cite{zwane}. In this section, the focus is on the latter two approaches.
...
\begin{align*}
  ...
  \text{logit}(\omega)  &= (e^{-\pi})\beta_{0} + \frac{1}{2}(\beta_{0}+\beta_{1})X,
\end{align*}
