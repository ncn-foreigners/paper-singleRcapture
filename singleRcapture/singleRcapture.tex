...
\citep[cf.][]{baffour-awuah2009}. Social scientists utilize advanced
...
\nlset{2} Store values from the previous step: 
    ...
    (the last assignment is omitted during the first iteration), and assign values in the current iteration  (where $\boldsymbol{o}$ denotes offset)
...
function since implementation of \pkg{VGAM} allows the user to supply
full constraint matrixes (cf. \citet{yee2015vgam}) while
\pkg{singleRcapture} ``forces'' the user to use formulas. We have
...
\pkg{VGAM} is done in \proglang{C} making the implementation of
\code{vglm} faster than that of \code{estimatePopsize}.
