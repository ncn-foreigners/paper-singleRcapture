...
bridge this gap by the \pkg{singleRcapture} and
\pkg{singleRcaptureExtra} packages which implements state-of-the-art
...
\pkg{VGAM}. However, estimation of parameters of a given truncated (and
...
custom modelsas well as diagnostics plots (e.g.~rootograms) with
parameters estimated using a modified IRLS algorithm implemented by us
to for estimation stability. Furthermore, as many R users are familiar
with \pkg{countreg} or \pkg{VGAM} we have implemented a lightweight
extension \pkg{singleRcaptureExtra} available through Github
...
methods, the \code{estimatePopsizeFit} function for estimating
...
to a implement custom model.

\section{Theoretical background}\label{sec-theory}

\subsection{How do we estimate population size with a single register?}

Let \(Y_{k}\) represent the number of times \(k\)-th unit was observed
in a register. Clearly, we only observe \(Y_{k}>\) and we do not know
...
which can be further plugged-in the estimator \eqref{eq-ht-estimator}.
...
\begin{itemize}
    \item Generalized Chao's \citep{chao1987estimating} and Zelterman's \citep{zelterman1988robust} estimators via logistic regression on variable $Z$ defined as $Z=1$ if $Y=2$ and $Z=0$ if $Y=1$ with $Z\sim b(p)$ where $b()$ is Bernoulli distribution and $p$ can be modelled for each unit $k$ by $\text{logit}(p_k)=\ln(\lambda_k/2)$ for Poisson parameter $\lambda_k=\boldsymbol{x}_{k}\boldsymbol{\beta}$ (for extension for covariates see \cite{chao-generalization} and \cite{zelterman})
    \begin{align}
        \hat{N} &= N_{obs}+
    ...
    \end{align}
    \item Zero-truncated and zero-one-truncated Poisson \citep[cf. ][]{zotmodels}, geometric, NB type II (NB2) regression where the untruncated distribution is parameterized as
    \begin{equation*}
    ...
    \end{equation*}
    \item Zero-truncated one-inflated (\code{ztoi}) modifications distributions where the new probability $\mathbb{P}^{\ast}$ measure is defined in terms of count data measure $\mathbb{P}$ with support on $\mathbb{N}\cup\{0\}$ as
    \begin{equation*}
    \mathbb{P}^{\ast}[Y=y]=
    \begin{cases}
    \mathbb{P}[Y=0] & y=0, \\
    ...
    \begin{equation*}
        \mathbb{P}^{\ast}[Y=y|Y>0]=\omega\mathcal{I}_{\{1\}}(y)+(1-\omega)\mathbb{P}[Y=y|Y>0].
    \end{equation*}
    \item One-inflated zero-truncated (\code{oizt}) modifications distributions where the new probability $\mathbb{P}^{\ast}$ measure is defined as
    \begin{equation*}
    ...
    \end{equation*}
    Note that \code{ztoi} and \code{oizt} distributions are equivalent as shown by \cite{bohning2023equivalence} but population size estimators are different.
\end{itemize}
...
\begin{itemize}
    \item Zero-truncated Hurdle model (\code{ztHurdle*}) for Poisson, geometric and NB2 is defined as
    \begin{equation*}
    ...
    \end{equation*}
    \item The Hurdle zero-truncated (\code{Hurdlezt*}) for Poisson, geometric and NB2 is defined as:
    \begin{align*}
        \mathbb{P}^{\ast}[Y=y]&=\begin{cases}
        \pi & y=1, \\
...
\(\alpha\) being heterogeneous across the population (specified in
...
of the \(\omega\) inflation parameter (in \code{ziot} or \code{oizt}) is
more convenient that the interpretation of the \(\pi\) probability
parameter (in Hurdle models). Additionally the interpretation of the
\(\lambda\) parameter in (one) inflated models conforms to the intuition
that given that unit \(k\) comes from the non-inflated part of the
population then it follows a poisson distribution (respectively
geometric or negative binomial) with the \(\lambda\) parameter (or
\(\lambda,\alpha\)), in hurdle models one loses that interpretation. It
is somewhat interesting is that the estimates from Hurdle zero-truncated
and one-inflated zero-truncated models are ``usually'' quite close to
one another, this however require more studies.

\subsection{Fitting method}\label{fitting-method}

As previously noted the \pkg{singleRcapture} package supports modelling
(linear) dependence on covariates of all parameters. To that end
a\textasciitilde modified IRLS algorithm is employed, full details are
available in \cite{VGAM-main}. In order to employ the algorithm
a\textasciitilde modified model matrix is created
\(\boldsymbol{X}_{\text{vlm}}\) at call to \code{estimatePopsize}. In
the context of the models implemented in \pkg{singleRcapture} this
...
semiparametric and nonparametric with the nonparametric being bootstrap
being the usual bootstrap algorithm which as argued in \cite{norrpoll}
and \cite{zwane}.

The idea of semiparametric bootstrap is to modify the usual bootstrap to
include the additional uncertainty due to the sample size being a random
variable. This type of bootstrap can be in short described as:
...
realistic than semiparametric bootstrap. The parametric bootstrap
...
\subsection{Testing marginal frequencies}
...
\begin{center}\includegraphics[width=6cm]{singleRcapture_files/figure-latex/rootogram-1} \includegraphics[width=6cm]{singleRcapture_files/figure-latex/rootogram-2} \end{center}
...
R> t(round(apply(dfb, 2, quantile)*100,4))
\end{CodeInput}
\begin{CodeOutput}
                          0%     25%     50%    75%    100%
...
from the data then population size will drop by about 4236.
...
\begin{CodeChunk}
\begin{CodeInput}
R> pop_size_stratas <- stratifyPopsize(basicModel)
R> pop_size_stratas[, c("name", "Observed", "Estimated", "logNormalLowerBound", "logNormalUpperBound")]
\end{CodeInput}
\begin{CodeOutput}
                             name Observed  Estimated logNormalLowerBound
1                  gender==female      398  3811.0911           2189.0443
2                    gender==male     1482  8879.2594           6090.7762
3                     age==<40yrs     1769 10506.8971           7359.4155
4                     age==>40yrs      111  2183.4535            872.0130
5  nation==American and Australia      173   708.3688            504.6086
6                    nation==Asia      284  2742.3147           1755.2548
7            nation==North Africa     1023  3055.2033           2697.4900
8          nation==Rest of Africa      243  2058.1533           1318.7466
9                 nation==Surinam       64  2386.4513            505.2457
10                 nation==Turkey       93  1739.8592            638.0497
   logNormalUpperBound
1             6902.133
2            13354.880
3            15426.455
...
\end{CodeOutput}
\begin{CodeInput}
R> pop_size_stratas_custom <- stratifyPopsize(
+   object  = basicModel,
...
+ )
R> 
R> pop_size_stratas_custom[, c("name", "Observed", "Estimated", "logNormalLowerBound", "logNormalUpperBound")]
\end{CodeInput}
\begin{CodeOutput}
                                        name Observed Estimated
1                             gender==female      398 3811.0911
2                               gender==male     1482 8879.2594
3  genderfemale:nationAmerican and Australia       67  328.8780
4    gendermale:nationAmerican and Australia      106  379.4908
5                    genderfemale:nationAsia       62  775.9054
6                      gendermale:nationAsia      222 1966.4093
7            genderfemale:nationNorth Africa      169  644.0545
8              gendermale:nationNorth Africa      854 2411.1488
9          genderfemale:nationRest of Africa       65  682.1776
10           gendermale:nationRest of Africa      178 1375.9757
11                genderfemale:nationSurinam       20  931.4677
12                  gendermale:nationSurinam       44 1454.9835
13                 genderfemale:nationTurkey       15  448.6079
14                   gendermale:nationTurkey       78 1291.2513
15                    genderfemale:age<40yrs      378 3169.8263
16                      gendermale:age<40yrs     1391 7337.0708
17                    genderfemale:age>40yrs       20  641.2648
18                      gendermale:age>40yrs       91 1542.1886
   logNormalLowerBound logNormalUpperBound
1            2275.6416           6602.1612
2            6745.5675          11877.8858
3             255.7692            430.3013
4             318.4739            458.0301
5             604.5269           1001.4205
6            1225.6476           3253.9046
7             517.5628            816.4495
8            2242.8856           2599.7970
9             527.2996            888.9423
10           1155.7975           1645.7331
11            234.3583           3895.6296
12            502.1052           4389.8894
13            241.6446            844.5624
14            836.6593           2018.2367
15           2617.4717           3858.4164
16           5543.4798           9905.3720
17            288.7316           1456.2658
18            899.8794           2694.5381
\end{CodeOutput}
\end{CodeChunk}
...
\code{"parametric", "semiparametric"} and \code{"nonparametric"}
